...
3. At the final clean commit where `Verify:` passes, record post and post a fresh evidence receipt. The rebuild receipt carries the request's exact occurrence and heading and leaves the checked body unchanged.
...
4. Run `bun run typecheck` and `bun test` (add `DELEGATE_RUN_PENDING=1 bun test` when pending tests are in scope). If they pass, move on. If either fails, fix and retry.
...
2. `bun run typecheck`
3. `bun test` (add `DELEGATE_RUN_PENDING=1 bun test` when pending tests are in scope)
4. `litespec validate` — confirm no structural regressions.
5. Post a fresh evidence receipt identifying the unit by exact heading and occurrence without editing the body.
...
- Queue bookkeeping is the narrow exception: post a fresh evidence receipt with the GitHub request identity.
